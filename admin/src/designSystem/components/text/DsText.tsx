...
interface IPropTypes {
  ...
  lineHeight?:
    ...
    | '28'
    | '30'
    | '32'; // base on tailwind and tailwind custom config
...
const DsText: FC<IPropTypes> = ({
  ...
  const fontSizeInRem = customFontSize
    ? customFontSize / baseFontSize + 'rem'
    : ''; // convert px to rem for custom font-size
  const lineHeightInRem = customLineHeight
    ? customLineHeight / baseFontSize + 'rem'
    : ''; // convert px to rem for custom line-height

  return (
    <span
      id={id}
      /* ${color ? `text-${color}` : ''} ${hoverColor ? `cursor-pointer hover:text-${hoverColor}` : ''} */
      className={`ds-text ${`text-${fontSize}`} font-${fontWeight} ${
        align ? `text-${align}` : ''
      } ${truncate ? 'block truncate' : ''} ${italic ? 'italic' : ''} ${
        transform || ''
      } ${lineHeight ? `leading-${lineHeight}` : ''} ${
        lineClamp ? `line-clamp-${lineClamp}` : ''
      } ${display || ''} ${onClick ? 'cursor-pointer' : ''} ${className}`}
      style={{
      ...
      ref={(node) => {
        ...
        if (node && customFontSize) {
          node.style.setProperty(
            'font-size',
            nonResponsive ? customFontSize + 'px' : fontSizeInRem,
            'important',
          );
        }
